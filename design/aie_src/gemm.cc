...
template <int M, int K, int N, int IS_RELU>
void GemmReluMKKNTwoAccsStream<M, K, N, IS_RELU>::filter(
  ...
  for (int i = 0; i <= M-2; i+=2) {
    ...
    for (int j = 0; j < N; j+=8) { // 2x v8float output per iter
      ...
      for (int k = 0; k <= K-8; k+=8) { // += input[k:k+8]*weight[k:k+8,n:n+16]
        MAC_ROW(in_row[k+0], in_row[k+K+0]); // += input[0]*weight[0:16]
        MAC_ROW(in_row[k+1], in_row[k+K+1]);
        MAC_ROW(in_row[k+2], in_row[k+K+2]);
        MAC_ROW(in_row[k+3], in_row[k+K+3]);
        MAC_ROW(in_row[k+4], in_row[k+K+4]);
        MAC_ROW(in_row[k+5], in_row[k+K+5]);
        MAC_ROW(in_row[k+6], in_row[k+K+6]);
        MAC_ROW(in_row[k+7], in_row[k+K+7]);
      } // K

      int k = K/8*8;
      if ((K&0x7) != 0) {
        for (int p = 0; p < (K&0x7); p++) {
          MAC_ROW(in_row[k+p], in_row[k+K+p]);
        }
      } // K
      ...
      aie::store_v(out_row_ptr, accv2); out_row_ptr += 8;
    } // N
...
#undef MAC_ROW
  ...
  GEMM_PROFILE_FOOTER("GemmReluMKKNTwoAccsStream");
}
