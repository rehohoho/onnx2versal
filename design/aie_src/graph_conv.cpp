...
ConvReluGraphTest<ConvReluScalarBHWC, 28, 24, 1, 2, 2, 5> fpscalar_bhwc(
  "fpscalar_bhwc", fpweights, fpbias, 
  "conv_fpin.txt", "conv_fpout_ConvReluScalarBHWC.txt");
ConvReluGraphTest<ConvReluScalarBHWC, 28, 24, 1, 2, 2, 5> fpscalar_bhwc_rand(
  "fpscalar_bhwc_rand", fpweights_rand, fpbias_rand, 
  "conv_fpin_rand.txt", "conv_fpout_ConvReluScalarBHWC_rand.txt");

ConvReluGraphTest<Conv5x5ReluBCHW, 28, 24, 1, 2, 2, 5> fpvector_bchw(
  "fpvector_bchw", fpweights, fpbias, 
  "conv_fpin.txt", "conv_fpout_Conv5x5ReluBCHW.txt");
ConvReluGraphTest<Conv5x5ReluBCHW, 28, 24, 1, 2, 2, 5> fpvector_bchw_rand(
  "fpvector_bchw_rand", fpweights_rand, fpbias_rand, 
  "conv_fpin_rand.txt", "conv_fpout_Conv5x5ReluBCHW_rand.txt");

ConvReluGmemParamGraphTest<ConvReluScalarGmemParamBHWC, 28, 24, 1, 2, 2, 5> fpscalar_gmemparam_bhwc(
  "fpscalar_gmemparam_bhwc", "conv_fpweights.txt", "conv_fpbias.txt", 
  "conv_fpin.txt", "conv_fpout_ConvReluScalarGmemParamBHWC.txt");
ConvReluGmemParamGraphTest<ConvReluScalarGmemParamBHWC, 28, 24, 1, 2, 2, 5> fpscalar_gmemparam_bhwc_rand(
  "fpscalar_gmemparam_bhwc_rand", "conv_fpweights_rand.txt", "conv_fpbias_rand.txt", 
...
int main(int argc, char ** argv) {
  ...
  adfCheck(fpscalar_gmemparam_bhwc.init(), "init fpscalar_gmemparam_bhwc");
  adfCheck(fpscalar_gmemparam_bhwc.run(ITER_CNT), "run fpscalar_gmemparam_bhwc");
	adfCheck(fpscalar_gmemparam_bhwc.end(), "end fpscalar_gmemparam_bhwc");
  adfCheck(fpscalar_gmemparam_bhwc_rand.init(), "init fpscalar_gmemparam_bhwc_rand");
  adfCheck(fpscalar_gmemparam_bhwc_rand.run(ITER_CNT), "run fpscalar_gmemparam_bhwc_rand");
	adfCheck(fpscalar_gmemparam_bhwc_rand.end(), "end fpscalar_gmemparam_bhwc_rand");
  return 0;
}
#endif
...
int main(int argc, char ** argv) {
  adfCheck(fpscalar_bchw.init(), "init fpscalar_bchw");
  get_graph_throughput_by_port(fpscalar_bchw, "plout[0]", fpscalar_bchw.plout[0], 1*2*24*24, sizeof(float_t), ITER_CNT);
	adfCheck(fpscalar_bchw.end(), "end fpscalar_bchw");

  adfCheck(fpscalar_bchw_rand.init(), "init fpscalar_bchw_rand");
  get_graph_throughput_by_port(fpscalar_bchw_rand, "plout[0]", fpscalar_bchw_rand.plout[0], 1*2*24*24, sizeof(float_t), ITER_CNT);
	adfCheck(fpscalar_bchw_rand.end(), "end fpscalar_bchw_rand");


  adfCheck(fpscalar_bhwc.init(), "init fpscalar_bhwc");
  get_graph_throughput_by_port(fpscalar_bhwc, "plout[0]", fpscalar_bhwc.plout[0], 1*2*24*24, sizeof(float_t), ITER_CNT);
	adfCheck(fpscalar_bhwc.end(), "end fpscalar_bhwc");

  adfCheck(fpscalar_bhwc_rand.init(), "init fpscalar_bhwc_rand");
  get_graph_throughput_by_port(fpscalar_bhwc_rand, "plout[0]", fpscalar_bhwc_rand.plout[0], 1*2*24*24, sizeof(float_t), ITER_CNT);
	adfCheck(fpscalar_bhwc_rand.end(), "end fpscalar_bhwc_rand");
  

  adfCheck(fpvector_bchw.init(), "init fpvector_bchw");
  get_graph_throughput_by_port(fpvector_bchw, "plout[0]", fpvector_bchw.plout[0], 1*2*24*24, sizeof(float_t), ITER_CNT);
	adfCheck(fpvector_bchw.end(), "end fpvector_bchw");

  adfCheck(fpvector_bchw_rand.init(), "init fpvector_bchw_rand");
  get_graph_throughput_by_port(fpvector_bchw_rand, "plout[0]", fpvector_bchw_rand.plout[0], 1*2*24*24, sizeof(float_t), ITER_CNT);
	adfCheck(fpvector_bchw_rand.end(), "end fpvector_bchw_rand");


  adfCheck(fpscalar_gmemparam_bhwc.init(), "init fpscalar_gmemparam_bhwc");
  get_graph_throughput_by_port(fpscalar_gmemparam_bhwc, "plout[0]", fpscalar_gmemparam_bhwc.plout[0], 1*2*24*24, sizeof(float_t), ITER_CNT);
	adfCheck(fpscalar_gmemparam_bhwc.end(), "end fpscalar_gmemparam_bhwc");

  adfCheck(fpscalar_gmemparam_bhwc_rand.init(), "init fpscalar_gmemparam_bhwc_rand");
  get_graph_throughput_by_port(fpscalar_gmemparam_bhwc_rand, "plout[0]", fpscalar_gmemparam_bhwc_rand.plout[0], 1*2*24*24, sizeof(float_t), ITER_CNT);
	adfCheck(fpscalar_gmemparam_bhwc_rand.end(), "end fpscalar_gmemparam_bhwc_rand");
  return 0;
}
#endif
