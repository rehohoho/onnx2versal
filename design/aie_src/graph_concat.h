 ...
 * 
 * @connections
 * @connect{pin[0:LCNT], H*INP_W*4}
 * @connect{pout[0], H*OUT_W*4}
 * @endconnections
 */
  ...
  public:
    ...
    ConcatGraph() { 
      ...
      for (int i = 0; i < LCNT; i++)
        adf::connect<adf::window<H*INP_W*4>> (pin[i], k[0].in[i]);
      
      // OUT_W <= H*INP_W
      adf::connect<adf::window<H*OUT_W*4>> (k[0].out[0], pout[0]);
    }
    ...
    ConcatTwiceGraph() { 
      ...
      if (CONCAT_CNT > 1) { // register kernel error if CONCAT_CNT=1
        for (int i = 0; i < CONCAT_CNT - 1; i++) {
          k[i] = adf::kernel::create_object<CONCAT<8, H, INP_W, INNER_OUT_W>>();
          adf::source(k[i]) = "concat.cc";
          adf::headers(k[i]) = {"concat.h"};
          adf::runtime<ratio>(k[i]) = 0.6;

          for (int j = 0; j < 8; j++)
            adf::connect<adf::window<H*INP_W*4>> (pin[i*8+j], k[i].in[j]);
          adf::connect<adf::window<H*INNER_OUT_W*4>> (k[i].out[0], klast.in[i]);
        }
      }
      
      int i = CONCAT_CNT - 1;
      // remainder intermediate concat
      k[i] = adf::kernel::create_object<CONCAT<LCNT_REM, H, INP_W, INNER_OUT_W>>();
      adf::source(k[i]) = "concat.cc";
      adf::headers(k[i]) = {"concat.h"};
      adf::runtime<ratio>(k[i]) = 0.6;
      ...
      adf::connect<adf::window<H*OUT_W*4>> (klast.out[0], pout[0]);
    }
