...
void QuantizeLinear<TT, INP_H, INP_W, OUT_W>::filter(
  ...
  for (int i = 0; i < INP_H; i++) {
    for (int j = 0; j < INP_W; j+=16) {
      ...
      window_writeincr(out, ((aie::accum<acc48,16>) acc1).to_vector<TT>(xbitshift + ybitshift));
    }
...
void QuantizeLinearFmulStream<TT, INP_H, INP_W, OUT_W>::filter(
	input_stream<float>* restrict in,
  output_stream<TT>* out
) {
  PROFILE_HEADER2;
  
  float inv_y_scale = inv(y_scale);
  ...
  acc.from_vector(aie::broadcast<int16_t,16>(y_zero), 0);

  for (int i = 0; i < INP_H; i++) {
    for (int j = 0; j <= INP_W-16; j+=16) {
      x1 = upd_v(x1, 0, readincr_v4(in));
      x1 = upd_v(x1, 1, readincr_v4(in));
      x1 = aie::mul((aie::vector<float,8>) x1, inv_y_scale);
      y = upd_w(y, 0, float2fix(x1, 0));

      x2 = upd_v(x2, 0, readincr_v4(in));
      x2 = upd_v(x2, 1, readincr_v4(in));
      x2 = aie::mul((aie::vector<float,8>) x2, inv_y_scale);
      y = upd_w(y, 1, float2fix(x2, 0));

      auto acc2 = aie::add((aie::accum<acc48,16>) acc, (aie::vector<int32,16>) y);
      writeincr_v16(out, ((aie::accum<acc48,16>) acc2).to_vector<TT>(0));
    }
